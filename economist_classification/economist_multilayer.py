...
activation = Activation('relu')(concatenated_tensor)

## next convolutional layer
conv_0 = Conv1D(num_filters[0], kernel_size=(filter_sizes[0]), padding='same', kernel_initializer='normal', activation='relu')(activation)
maxpool_0 = MaxPooling1D(pool_size=(2), strides=1, padding='valid')(conv_0)
pooled_conv_0 = Dropout(drop)(maxpool_0)
conv_1 = Conv1D(num_filters[0], kernel_size=(filter_sizes[1]), padding='same', kernel_initializer='normal', activation='relu')(activation)
maxpool_1 = MaxPooling1D(pool_size=(2), strides=1, padding='valid')(conv_0)
pooled_conv_1 = Dropout(drop)(maxpool_0)
conv_2 = Conv1D(num_filters[0], kernel_size=(filter_sizes[2]), padding='same', kernel_initializer='normal', activation='relu')(activation)
maxpool_2 = MaxPooling1D(pool_size=(2), strides=1, padding='valid')(conv_0)
pooled_conv_2 = Dropout(drop)(maxpool_0)
...
output = Dense(num_labels+1, activation='softmax')(dropout)
...
model.save('multilayer_economist_CNN_Glove_II.h5')
