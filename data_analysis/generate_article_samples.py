...
'''
generate some articles to look at
'''
...
sample = np.random.randint(0, len(labels), 10)
c = 0;
for i in sample:
    sample_title = 'economist_sample_'+str(c)+'.txt';
    article = data[i];
    #write article to text file;
    text_file = open(settings.ROOT_DIR+'\\article_samples\\'+sample_title, "w", encoding = 'UTF-8')
