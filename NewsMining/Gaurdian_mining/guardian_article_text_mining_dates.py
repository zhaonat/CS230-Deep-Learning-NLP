...
for article in data.T.columns:
    ...
    if(isinstance(text, str)):
        ...
        date = data.T[article]['date_publication']
        ## extract the year from format year-month-dayTime 2009-12-10T23:20:00Z
        split = date.split('-'); #enough to get the year
        year = split[1]; print(year)
        date_label.append(year)
        doc_counter += 1
    else:
...
file = open(out_dir+'Guardian_epoch_4_with_month_labels.p', 'wb')
pickle.dump([preprocessed_docs, date_label], file);

file = open(out_dir+'Guardian_raw_epoch_4_with_month_labels.p', 'wb')
pickle.dump([raw_preprocessed, date_label], file);
