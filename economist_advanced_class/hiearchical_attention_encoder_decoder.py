...
## filter out LABELS
counter = 0; remove_indices = list();
#useless labels are labels whose topic content is hard to discern or have too few examples
#see the corpus_label_analysis.py
#most of these are small (less than 100, the largest is probably News Summaries at 346...
useless_labels = ['Christmas Specials', 'Diary', 'By Invitation', 'Moreover', 'News Summaries', 'Review', 'Science brief',\
                  'Business books', 'Essay', 'The World If', 'Economics brief', 'Special', 'Schools brief', 'World this week']
for label in labels:
    if(label in useless_labels):
        remove_indices.append(counter);
    #concatenate the brief and briefing labels
    if(label == 'Briefing'):
        labels[counter] = 'Brief';
    if(label == 'China'):
        labels[counter] = 'Asia';
...
econ_corpus = [i for j, i in enumerate(econ_corpus) if j not in remove_indices];
